...
# mod_back and mod_forw are the same model 
...
percent_diff <- ((-4.538e-06 + 5.891e-06)/-5.891e-06)*100
percent_diff
...
mod_r <- glm(pap_f ~ race_f, family=binomial(), data=cc_df)
summary(mod_r)
...
app_diff_h <- 100*((0.35353-0.62993)/0.35353)
app_diff_h
app_diff_b <- 100*((0.39701-0.54115)/0.39701)
app_diff_b
app_diff_a <- 100*((1.13762-1.13644)/1.13762)
app_diff_a
app_diff_o <- 100*((0.26861-0.35428)/0.26861)
app_diff_o

# Testing for effect modification between race and insurance coverage 
anova(mod_em_ri1, mod_em_ri, test="Chisq")
```
...
Confounding is present between every race and insurance coverage except for those who identify as Asian, so there may be confounding between race in general and insurance coverage. 
