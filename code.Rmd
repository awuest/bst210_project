...
h209red <- h209red %>% 
  mutate(genhlth_avg_f = factor(genhlth_avg, 
                                  levels = c("5", "4", "3", "2", "1"))) %>%
  mutate(genhlth_avg = fct_recode(genhlth_avg_f,
                                    "poor" = "5",
                                    "fair" = "4",
                                    "good" = "3",
                                    "very good" = "2",
...
# perceived avg general helath as factor
...
```{r}
# cost related
m1 <- glm(formula = pap_f ~ afford_care_f + inscov_gen_2018_f + income_indiv + income_fam + income_percpov, family="binomial", data = h209red)
summary(m1)

# accessibility related
m2 <- glm(formula = pap_f ~limitation_f + have_usc_f + dist_from_usc_f + usc_offhrs_nw_f + rch_usc_byphn_f, family="binomial", data = h209red)
summary(m2)

# provider characteristics
m3 <- glm(formula = pap_f ~ usc_asks_abt_trts_f + usc_asks_hlp_dec_f + usc_expln_options_f + usc_spk_lang_f , family="binomial", data = h209red)
summary(m3)

#demographics
m4 <- glm(formula = pap_f ~ region_f , family="binomial", data = h209red)
summary(m4)

m4 <- glm(formula = pap_f ~  race_f , family="binomial", data = h209red)
summary(m4)

m4 <- glm(formula = pap_f ~  age , family="binomial", data = h209red)
summary(m4)

m4 <- glm(formula = pap_f ~ marital_stat_f, family="binomial", data = h209red)
summary(m4)

m4 <- glm(formula = pap_f ~ educ_f, family="binomial", data = h209red)
summary(m4)
```
...
mod1 <-glm(pap_f~., family = "binomial", data=h209red)
full.model <- glm(formula = pap_f ~ region_f + race_f + marital_stat_f + educ_f + smoke_freq_f + income_indiv + income_fam + income_percpov + menhlth_avg + genhlth_avg + limitation_f + afford_care_f + have_usc_f + dist_from_usc_f +rch_usc_byphn_f + usc_offhrs_nw_f + usc_asks_abt_trts_f + usc_asks_hlp_dec_f + usc_expln_options_f + inscov_gen_2018_f + totexp + outofpocket_exp , family = "binomial", data=mod1$model)


mod_back <- step(full.model, direction = "backward", na.rm=TRUE)
summary(mod_back)
```