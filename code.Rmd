...
m4 <- glm(formula = pap_f ~  race_f, family = binomial(), data = df)
summary(m4)

m4 <- glm(formula = pap_f ~  age, family = binomial(), data = df)
summary(m4)

m4 <- glm(formula = pap_f ~ marital_stat_f, family = binomial(), data = df)
summary(m4)

m4 <- glm(formula = pap_f ~ educ_f, family = binomial(), data = df)
summary(m4)
```
...
mod_forw <- step(glm(pap_f ~ 1, data = cc_df, family = binomial()), ~ age + income_indiv + income_fam + totexp + outofpocket_exp + menhlth_avg_f + genhlth_avg_f + region_f + race_f + marital_stat_f + educ_f + smoke_freq_f + limitation_f + afford_care_f + have_usc_f + usc_access_score + usc_satisf_score + inscov_gen_2018_f, direction = "forward")
summary(mod_forw)

full_mod <- glm(pap_f ~ age + income_indiv + income_fam + totexp + outofpocket_exp + menhlth_avg_f + genhlth_avg_f + region_f + race_f + marital_stat_f + educ_f + smoke_freq_f + limitation_f + afford_care_f + have_usc_f + usc_access_score + usc_satisf_score + inscov_gen_2018_f, family = binomial(), data = cc_df) 
summary(full_mod)

mod_back <- step(full_mod, direction = "backward")
summary(mod_back)

hoslem.test(as.numeric(cc_df$pap_f) - 1, fitted(mod_forw), g = 10)
