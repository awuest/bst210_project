...
fit_ii_quad_pred_metrics$auc # slightly worse auc from interaction 1 so we should not keep
...
fit_ii_spline_pred_metrics$auc # slightly worse auc from interaction adjusted model so we should not keep
...
fit_ii_gam_pred_metrics$auc # slightly worse than interaction adjusted model so we should not keep 
...
# incorporating gamma out of pocket expense
...
hoslem.test(train_set$pap_num, fitted(fit_oop_gam), g = 10) # good fit to training

# now we can see how it performs on testing data
fit_oop_gam_pred_metrics <- pred_metrics(fit_oop_gam, 0.26)
fit_oop_gam_pred_metrics$auc #slightly better auc than the adjusted model with spline total exp so we could keep
...
df_NA <- df_NA[complete.cases(df_NA),]

# still over 4000 rows left
fit_rf <- randomForest(pap_f ~., nodesize=1000, data=df_NA)
