...
fit_ii_spline_pred_metrics$auc # slightly worse auc from interaction 1 so we could keep
```
...
# spline term has better auc so let's keep that
```

```{r}
# incorporating non-linear tout of pocket expense
# quadratic out of pocket expense
fit_oop_quad <- glm(formula = pap_num ~ age + I(age^2) + income_indiv + income_fam + totexp + I(totexp^2) + outofpocket_exp + I(outofpocket_exp^2) + genhlth_avg_f + region_f + race_f + marital_stat_f + educ_f + smoke_freq_f + limitation_f + afford_care_f + have_usc_f + inscov_gen_2018_f + marital_stat_f*income_fam + educ_f*totexp, family = binomial(), data = train_set)
...
fit_oop_quad_pred_metrics$auc # slightly worse auc from total expense adjusted model so we won't keep

# incorporating spline total expense
fit_oop_spline <- glm(formula = pap_num ~ age + I(age^2) + income_indiv + income_fam + totexp + I(totexp^2) + outofpocket_exp + bSpline(df=6, degrees=3, outofpocket_exp) + genhlth_avg_f + region_f + race_f + marital_stat_f + educ_f + smoke_freq_f + limitation_f + afford_care_f + have_usc_f + inscov_gen_2018_f + marital_stat_f*income_fam + educ_f*totexp, family = binomial(), data = train_set)

hoslem.test(train_set$pap_num, fitted(fit_oop_spline), g = 10) # better fit with the spline term when compared to the model adjusted with total expense 
...
fit_oop_gam <- gam(formula = pap_num ~ age + I(age^2) + income_indiv + income_fam + totexp + I(totexp^2) + outofpocket_exp + s(outofpocket_exp,4) + genhlth_avg_f + region_f + race_f + marital_stat_f + educ_f + smoke_freq_f + limitation_f + afford_care_f + have_usc_f + inscov_gen_2018_f + marital_stat_f*income_fam + educ_f*totexp, family = binomial(), data = train_set)


hoslem.test(train_set$pap_num, fitted(fit_oop_gam), g = 10) # better fit with the quadratic term when compared to the model adjusted with total expense 
# now we can see how it performs on testing data
